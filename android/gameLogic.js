...
export const quarterFinals = async () => {
  ...
  for (let i = 0; i < users.length; i += 16) {
    ...
    tournament.sort((a,b) => a.quarterFinals[0] - b.quarterFinals[0])
    let winners = [];
...
export const semiFinals = async () => {
  ...
  for (let i = 0; i < users.length; i += 16) {
    ...
    tournament.sort((a,b) => a.semiFinals[0] - b.semiFinals[0])
    let winners = [];
    for (let k = 0; k < tournament.length; k += 2) {
      var result = await matchResult(
        tournament[k].squad,
      ...
      tournament[k].semiFinals[1] = result[0];
      tournament[k].semiFinals[2] = result[1];
      tournament[k + 1].semiFinals[1] = result[1];
      tournament[k + 1].semiFinals[2] = result[0];
      try {
        await tournament[k].save();
        await tournament[k + 1].save();
      } catch (error) {
...
export const final = async () => {
  ...
  for (let i = 0; i < users.length; i += 16) {
    ...
    for (let k = 0; k < tournament.length; k += 2) {
      ...
      tournament[k].finale[1] = result[0];
      tournament[k].finale[2] = result[1];
      tournament[k + 1].finale[1] = result[1];
      tournament[k + 1].finale[2] = result[0];
      try {
        await tournament[k].save();
        await tournament[k + 1].save();
      } catch (error) {
